...
get_ipython().run_line_magic("pip", "install ftfy regex tqdm --user")
get_ipython().run_line_magic("pip", "install pandas --user")
get_ipython().run_line_magic("pip", "install wordcloud --user")
get_ipython().run_line_magic("pip", "install sklearn --user")
get_ipython().run_line_magic("pip", "install scikit-learn --user")
get_ipython().run_line_magic("pip", "install Levenshtein --user")
...
def split_train_test(dataframe, test_size=0.05):
    ...
    train, test = train_test_split(
        ...
        test_size=test_size,
        random_state=42,
        stratify=dataframe[["answer_type", "answerable"]],
    )
    return train, test
...
ANSWER_SPACE = get_number_of_distinct_answers(
    train_df
)  # The answer space will be decreased later when we process the answers
...
validation_images = process_images(
    validation_df, VALIDATION_PATH, clip_model, preprocessor, DEVICE
)
...
class VizWizDataset(Dataset):
    def __init__(
        ...
        self.answer_onehotencoded = answer_onehotencoder.transform(
            self.copied_dataframe[["answer"]]
        ).toarray()
        self.answer_type_onehotencoded = answer_type_onehotencoder.transform(
            self.copied_dataframe[["answer_type"]]
        ).toarray()
    ...
    def __getitem__(self, index):
        ...
        answer_type = torch.tensor(
            self.answer_type_onehotencoded[index], dtype=torch.float32
        )
        answer_counter = torch.tensor(
            self.answer_counter_per_question[index], dtype=torch.long
        )
        answerable = torch.tensor(self.answerable[index], dtype=torch.float32)
        return (
            self.images_features[index],
...
class VQAModel(nn.Module):
    ...
    def train_model(
        ...
        for epoch in range(1, epochs + 1):
            (
                training_loss,
                training_accuracy,
                training_vizwiz_accuracy,
                train_answerability_score,
            ) = self.training_step(
                training_dataloader, criterion, optimizer, self.device
            )
            (
                validation_loss,
                validation_accuracy,
                validation_vizwiz_accuracy,
                validation_answerability_score,
            ...
            print(
                "Epoch: {} | Training Accuracy: {:.3f} | Validation Accuracy: {:.3f} | Test Accuracy: {:.3f}".format(
                    epoch, training_accuracy, validation_accuracy, test_accuracy
                )
            )
            print(
                "Epoch: {} | Training VizWiz Accuracy: {:.3f} | Validation VizWiz Accuracy: {:.3f} | Test VizWiz Accuracy: {:.3f}".format(
                    epoch,
                    training_vizwiz_accuracy,
                    validation_vizwiz_accuracy,
                    test_vizwiz_accuracy,
                )
            )
            print(
                "Epoch: {} | Training Answerability Score: {:.3f} | Validation Answerability Score: {:.3f} | Test Answerability Score: {:.3f}\n".format(
                    epoch,
                    train_answerability_score,
                    validation_answerability_score,
                    test_answerability_score,
                )
            )
    ...
    def training_step(self, dataloader, criterion, optimizer, device):
        ...
        for _, batch in enumerate(dataloader):
            image, question, answer, answer_type, answers_for_questions, answerable = (
                batch
            )
            image, question, answer, answer_type, answers_for_questions, answerable = (
                image.to(device),
                ...
                answers_for_questions.to(device),
                answerable.to(device),
            )
            optimizer.zero_grad()
            output, answer_type_predicted, answerable_predict = self.forward(
                image, question
            )
            ...
            for i in range(len(answer)):
                ...
                vizwiz_accuracy += min(
                    1,
                    torch.sum(
                        torch.eq(predicted_answer[i], answers_for_questions[i])
                    ).item()
                    / 3,
                )
    ...
    def validation_step(self, dataloader, criterion, device):
        validation_loss, validation_accuracy, vizwiz_accuracy, total_sum = (
            0.0,
            0.0,
            0.0,
            0,
        ...
        with torch.no_grad():
            for _, batch in enumerate(dataloader):
                ...
                (
                    ...
                    answers_for_questions.to(self.device),
    ...
    def test_model(self, image_path, question):
        ...
        predicted_answer, predicted_answer_type, answerability = self.predict(
...
training_dataset = VizWizDataset(
    train_df, None, None, MODEL_NAME, training_images, training_questions
)
...
model = VQAModel(
    num_classes=NUM_CLASSES, device=DEVICE, hidden_size=512, model_name=MODEL_NAME
).to(DEVICE)
...
IMAGE_PATH = "/kaggle/input/vizwiz/train/train/VizWiz_train_00000008.jpg"
...
for i in range(len(df)):
    ...
    image_path = "/kaggle/input/vizwiz/test/test/" + image_url
    predicted_answer, predicted_answer_type, answerability = model.test_model(
        image_path=image_path, question=question
    )
